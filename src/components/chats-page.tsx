...
export default function ChatsPage({ onBack, onSelectChat, currentSessionId, sessions }: ChatsPageProps) {
  const [searchQuery, setSearchQuery] = useState('');
  const [sortBy, setSortBy] = useState<'recent' | 'oldest' | 'alphabetical'>('recent');

  // Filter and sort sessions based on search query and sort option
    ...
    <div className="h-screen bg-custom-dark overflow-hidden">
      ...
      <div className="border-b border-border p-4">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-text-secondary">Search and manage your conversations</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border-b border-border">
        ...
        <div className="mt-3 text-sm text-text-secondary">
          {searchQuery ? (
            <span>
              {filteredAndSortedSessions.length} of {sessions.length} chats match "{searchQuery}"
            </span>
          ) : (
        ...
        {filteredAndSortedSessions.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-text-secondary">
              {searchQuery 
                ? 'Try adjusting your search terms or create a new chat'
